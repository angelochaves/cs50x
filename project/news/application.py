...
def index():
    ...
    if request.method == "POST":
        #result = search(request.form.get("keyword"))
...
@app.route("/results")
def results():
    """Show results and able user to save it in user's history page"""

    if request.method == "POST":

        exemp = 0# insert result fields in database
